    ...
    <>
      ...
      <div className="eink-root">
                ...
                <tbody>
                  {events.map((event, i) => (
                    <tr key={`${event.start.toISOString()}-${i}`}>
                      <td className="eink-idx">{String(i + 1).padStart(2, "0")}</td>
                      <td>
                        <p className="eink-when">
                          {formatEventDay(event.start, now, config.timeZone)} ·{" "}
                          {formatEventTime(
                            event.start,
                            event.allDay,
                            config.timeZone,
        ...
        <p className="eink-footer">
          refresh {config.refreshSeconds}s · {config.timeZone}
        </p>
      </div>
    </>
  );
